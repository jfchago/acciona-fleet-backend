...
public class CarFleetRequestController {
    ...
    @PatchMapping(path="/{id}",consumes=MediaType.APPLICATION_JSON_VALUE) public ResponseEntity<UpdateResponse> update(@PathVariable Long id,@RequestHeader(HttpHeaders.IF_MATCH) String ifMatch,@RequestBody Map<String,Object> changes){var result=useCases.update(id,version(ifMatch),changes);return ResponseEntity.ok().eTag(etag(result.request())).body(new UpdateResponse(ItemResponse.from(result.request()),result.warnings()));}
    ...
    private static String etag(CarFleetRequest r){return "\""+r.version()+"\"";} private static String version(String e){if(e==null)throw new IllegalArgumentException("If-Match is required");return e.replace("\"","");}
